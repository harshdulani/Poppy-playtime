...
namespace YsoCorp {
    namespace GameUtils {
        ...
        public class YCMenuItemManager {
            ...
            [MenuItem("GameUtils/Open game's page", false, 1)]
            ...
            [MenuItem("GameUtils/Update GameUtils", false, 101)]
            static void MenuUpdateGameutils() {
                UpdateGameutils();
            }

            [MenuItem("GameUtils/Upgrade Applovin", false, 102)]
            ...
            [MenuItem("GameUtils/Import Config", false, 103)]
            ...
            private static IEnumerator ImportGameutilsPackage(GUUpdateData data) {
                var path = Path.Combine(Application.temporaryCachePath, "Gameutils_" + data.version.Replace('.', '_'));
                    ...
                    if (MAX_UPGRADING) { // Anything needs an update
                        ...
                        if (dialog) {
                            ...
                            for (int i = 0; i < tmpNetworks.Count; i++) {
                                ...
                                YCEditorCoroutine.StartCoroutine(UpgradeMaxNetwork(tmpNetworks[i], () => MAX_NETWORKS_UPGRADING.Remove(currentNetwork)));
                            }
                            if (MAX_MAIN_UPGRADING != null) {
                                YCEditorCoroutine.StartCoroutine(UpgradeMaxMainPlugin(MAX_MAIN_UPGRADING));
                            }
                        } else {
            ...
            private static IEnumerator UpgradeMaxNetwork(Network network, Action action) {
                var path = Path.Combine(Application.temporaryCachePath, network.DisplayName);
                var downloadHandler = new DownloadHandlerFile(path);
...
#endif          
                {
                    Debug.LogError("The network " + network.DisplayName + " could not be upgraded.");
                } else {
            ...
            private static IEnumerator UpgradeMaxMainPlugin(Network applovin) {
                while (MAX_NETWORKS_UPGRADING.Count != 0) {
                    yield return new WaitForSeconds(0.1f);
                }
                YCEditorCoroutine.StartCoroutine(UpgradeMaxNetwork(applovin, () => MAX_MAIN_UPGRADING = null));
            }
            #endregion
